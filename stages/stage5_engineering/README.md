...
## 2. Ablation 5.1 — 是否扩词表

> 期望差距 < 0.5pt → 不扩。

```bash
# Arm A: 不扩（已经是 stage1 默认）
# Arm B: 扩 200 token（脚本需自行实现：在法律语料跑 BPE → 选 top-200 → resize embedding）
python stages/stage5_engineering/ablations/extend_vocab.py \
  --base Qwen/Qwen3-8B \
  --corpus data/processed/sft_demo/clean.jsonl \
  --new-token-count 200 \
  --output ckpts/qwen3-8b-vocab-ext

deepspeed --num_gpus=4 stages/stage1_sft_grpo/train_sft.py \
  --model_name_or_path ckpts/qwen3-8b-vocab-ext \
  --dataset_path       data/processed/sft_demo \
  --output_dir         ckpts/legalgpt-8b-vocab-ext-sft \
  --swanlab_run_name stage5-arm-b-vocab-ext
```

详细方法见 [`ablations/vocab_extension.md`](ablations/vocab_extension.md)。
